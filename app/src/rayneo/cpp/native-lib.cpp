...
namespace {
struct AppContext {
    crow::DeviceDelegateRayNeoPtr mDevice;
...
(JNIEnv* aEnv, jobject aActivity, jobject aAssetManager) {
  ...
  sAppContext->mDevice = crow::DeviceDelegateRayNeo::Create(BrowserWorld::Instance().GetRenderContext());
  sAppContext->mDevice->Resume();
...
(JNIEnv*, jobject, jboolean aDown, jfloat aX, jfloat aY) {
  //TODO Kaidi 注意，当控制器事件OK后，要注释掉这里的触摸事件逻辑
  //因为已经不需要触摸事件来交互了
...
(JNIEnv*, jobject, float f0, float f1, float f2, float f3) {
    sAppContext->mDevice->SetDeviceQuaternion(f0, f1, f2, f3);
}
JNI_METHOD(void, setControllerQuaternion)
(JNIEnv*, jobject, float f0, float f1, float f2, float f3) {
    sAppContext->mDevice->SetControllerQuaternion(f0, f1, f2, f3);
}
